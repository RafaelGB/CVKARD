...
public class RootController {
	...
	public String curriculum(@PathVariable("nick") String nick, HttpSession session, Model model) {
		String url = "index";
	...
	/**
	 * ???????????????????????? Contact - Vista formulario de contacto entre
	 * usuarios
	 */
	@GetMapping("/contacto")
	...
	@Transactional
	public String showmessage(HttpSession session, Model model, @PathVariable("type") String type,
			@PathVariable("id") long id) {
		String url = "home";
		boolean correct = false;
		Message m = entityManager.find(Message.class, id);// refresh de la base
															// de datos
		User u = (User) session.getAttribute("user");
		if (type.equals("R")) {// comprobamos si está en recibidos
			model.addAttribute("entry", true);
			if (m.getReceiver().getId() == u.getId()) {// si existe dentro de
														// los recibidos lo
														// habilitamos
				correct = true;
				if (m.getSender() != null)
			...
			if (m.getSender().getId() == u.getId()) {// si existe dentro de los
														// enviados lo
														// habilitamos
				correct = true;
	...
	@Transactional
	public String buzon(HttpSession session, HttpServletResponse response, @PathVariable("pag") String pag,
		...
		if (type.equals("N")) {
			...
			try {
				...
				if (pag.equals("1")) {
					...
					u = entityManager.find(User.class, u.getId());// refresh de
																	// la base
																	// de datos
	...
	/**
	 * PerfilEmpresa - vista sobre el perfil de un negocio en concreto con sus
	 * configuraciones
	 */
	@GetMapping("/perfilempresa")
	public String perfilempresa(HttpSession session) {
		User u = (User) session.getAttribute("user");
		u = entityManager.find(User.class, u.getId());// refresh de la base de
														// datos
	...
	/**
	 * PerfilUsuario - vista sobre el perfil de un aspirante a trabajador en
	 * concreto con sus configuraciones
	 */
	...
	/**
	 * TablaProyectos - vista sobre los proyectos de un trabajador en concreto
	 * bajo su log y sus opciones de edición
	 */
	@GetMapping("/tablaproyectos/{type}/{pag}")
	@Transactional
	public String tablaproyectos(Model model, @PathVariable("pag") String pag, @PathVariable("type") String type,
			HttpSession session) {
		String exit = "home";
		if (type.equals("N")) {
			model.addAttribute("type", type);
	...
	@Transactional
	public String editOffer(HttpSession session, Model model, @PathVariable("id") long id) {
	...
	/**
	 * TablaOfertas - vista sobre las ofertas de un negocio en concreto bajo su
	 * log y sus opciones de edición
	 */
	@GetMapping("/tablaofertas/{pag}")
	@Transactional
	public String tablaofertas(HttpSession session, HttpServletResponse response, @PathVariable("pag") String pag,
			Model model) {
		String exit = "home";
	...
	/**
	 * OfertaVista - vista sobre la información de una oferta en concreto y sus
	 * referencias
	 */
	...
	@Transactional
	public String ofertas(HttpSession session, HttpServletResponse response, @PathVariable("pag") String pag,
			Model model) {
		String exit = "home";
	...
	@Transactional
	public String language(Model model, @PathVariable("id") Long id, @PathVariable("pag") String pag,
			HttpSession session) {
		String exit = "home";
