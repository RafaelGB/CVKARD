    ...
    <section>
      ...
      <div id="content">
        <div class="line">
					...
					<a href="/Ls/N" id="new" class="button alt fit small">Nuevo</a>
					...
					<div class="box">
						...
						<div class="row">
							...
							<c:choose>
								<c:when test="${pag=='N'}">
									<form method="post" action="/Language/createL">
										<div class="row">
											<div class="u">
												<div class="column1">
				    ...
				    <c:otherwise>
						...
						<form action="/Language/actionOnMarks" method="POST">
		                  <table>
		                    <thead>
		                      <tr>
				           ...
				           <tbody>
		                     <c:forEach items="${Ls}" var="l" begin="${(pag-1)*10}" end="${((pag-1)*10)+9}"> 
		                      	<tr>
			                      	<td style="width: 10%" class="selec">
			                          <input type="checkbox" id="copy${l.id}" name="checked" value="${l.id}"></input>
			                          <label for="copy${l.id}"></label>
   			                        </td>
   			                        <td style="width: 30%"><a href=""></a></td>
			                        <td style="width: 50%"><a href="">${l.name}</a></td>
		                      	</tr>
		                     </c:forEach>
		                    </tbody>
		            	...
		            	<div class="w3-bar w3-border w3-round">
											<div class="w3-bar">
												<a href="/Ls/${pag>=5 ? pag-4 : 1}"
													class="w3-bar-item w3-button"> &laquo; </a> <a
													href="/Ls/${pag}" class="w3-button">
													${pag} </a> <a
													href="/Ls/${(((pag)*10)+1) <= size ? pag+1 : pag}"
													class="w3-button"> ${(((pag)*10)+1) <= size ? pag+1 : '-'}
												</a> <a
													href="/Ls/${(((pag+1)*10)+1) <= size ? pag+2 : pag}#"
													class="w3-button">${(((pag+1)*10)+1) <= size ? pag+2 : '-'}
												</a> <a
													href="/Ls/${(((pag+2)*10)+1)<= size ? pag+3 : pag}#"
													class="w3-button">${(((pag+2)*10)+1) <= size ? pag+3 : '-'}
												</a> <a
													href="/Ls/${(((pag+3)*10)+1) <= size ? pag+4 : pag}#"
