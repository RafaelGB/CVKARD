      ...
      <div id="content">
        <div class="line">
          <section id="main" class="container"> 
              ...
              <a href="/home"> Home > </a> <a id="actual">Lenguaje </a>

          	</div>

            <div class="box">
            <h2>Proyectos que contienen este lenguaje</h2>
              <div class="row">
             ...
             <div class="w3-bar w3-border w3-round">
		                    <div class="w3-bar">
		                      <a href="/language/${lang.id}/${pag}>=5 ? pag-4 : 1}" class="w3-bar-item w3-button"> &laquo; </a>
		                      <a href="/language/${lang.id}/${pag}" class="w3-button"> ${pag} </a>
		                      <a href="/language/${lang.id}/${(((pag)*4)+1) <= size ? pag+1 : pag}" class="w3-button"> ${(((pag)*4)+1) <= size ? pag+1 : '-'} </a>
		                      <a href="/language/${lang.id}/${(((pag+1)*4)+1) <= size ? pag+2 : pag}#" class="w3-button">${(((pag+1)*4)+1) <= size ? pag+2 : '-'} </a>
		                      <a href="/language/${lang.id}/${(((pag+2)*4)+1)<= size ? pag+3 : pag}#" class="w3-button">${(((pag+2)*4)+1) <= size ? pag+3 : '-'} </a>
		                      <a href="/language/${lang.id}/${(((pag+3)*4)+1) <= size ? pag+4 : pag}#" class="w3-button"> &raquo; </a>
		                    </div>
		     </div>
          </section>
       </div>
